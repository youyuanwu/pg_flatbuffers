// pgrx-side SQL tests for the `super` entry points.
//
// `include!`d from `super::mod.rs` inside a `#[pg_schema] mod tests
// { ... }` block (the proc-macro requires an inline mod body), so
// inner doc comments and `mod` declarations are not available
// here. Anything `pub` from this file is therefore scoped to that
// inline mod.
...
fn pg_query_multi_empty_buf_returns_zero_rows() {
    let n =
        Spi::get_one::<i64>("SELECT count(*) FROM flatbuffers_query_multi('B:tags[*]', ''::bytea)")
            .expect("SPI failure")
            .expect("count is non-null");
    assert_eq!(n, 0);
}
